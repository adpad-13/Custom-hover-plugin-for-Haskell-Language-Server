...
* **Dictionary Filtering:** If you write a typeclass constraint, GHC silently generates hidden dictionary variables (like `$dEq`). My plugin filters out anything starting with `$` to keep the output clean.
...
```
To use your cloned hls for a project ,
run :
```
